...
// WiFi Credentials - replace with your 
own const char* ssid = "F22";   
const char* password = "mdab23092004";   
// Web Server setup   
WebServer server(80);   
// Define pins for flex sensors  const int flexPins[] = {32,35,34,39,36};  // ADC pins on ESP32 const int numSensors = 5;   
...
// MPU6050 object  MPU6050 mpu;   
// Struct to name parameters moreexplicitly 
...
void handleRoot() {   
String html = String("<!DOCTYPE html>") + "<html lang=\"en\">" + "<head>" + "<meta charset=\"UTF-8\">" + "<meta name=\"viewport\" content=\"width=device-width,initialscale=1.0\">" + "<title>Gesture Glove Interface</title>" + "<style>" + "body { font-family: Arial, sans-serif; background-color: #f0f0f0; display:flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }" + ".container { background-color: white; border-radius: 10px; box-shadow: 04px 6px rgba(0,0,0,0.1); padding: 20px; text-align: center; }" +   
".gesture-display { font-size: 2em; margin: 20px; color: #333; }" +   
"button { background-color: #4CAF50; color: white; border: none; padding: 
10px   
20px; margin: 10px; border-radius: 5px; cursor: pointer; }" +   
"</style>" +    
"</head>" +   
"<body>" +   
"<div class=\"container\">" +   
...
void handleGesture() {    server.send(200, 
"text/plain", lastDetectedGesture);  }   
void handleSpeak() {   
...
void calibrateSensors() {   
// Take multiple readings for more accurate  
calibration   const int numReadings = 10;   
...
calculateResistance(analogRead(flexPins[i]));     float ratio = currentResistance 
/ baselineValues[i];   
sensor index     switch(i) {       case 0: 
currentGesture.thumb = ratio; break;       case 
1: currentGesture.index = ratio; break;       
...
// Calculate angle in degrees using 
arctangent   // This calculates the angle from 
...
float voltage = (adcValue * 3.3) / 4096.0;  // Convert ADC to  
  
voltage   float flexResistance = BRIDGE_RESISTANCE * (3.3 / voltage - 1.0);   return flexResistance;   
}   
void checkGestures() {       
// All fingers bent    bool gesture1 = 
(currentGesture.index > 1.05 &&                     
...
// Gesture A: 4 fingers (except thumb) bent, thumb 
extended   bool gesture2 = ( 
currentGesture.index > 1.05 &&                    
currentGesture.middle > 1.05 &&                    
currentGesture.ring > 1.05 &&                     
currentGesture.pinky > 1.05 &&                     
currentGesture.thumb < 1.05 &&   
currentGesture.angle >= 75 && currentGesture.angle <= 100);   
// Gesture B: Index and thumb straight, others 
bent   bool gesture3 = ( 
currentGesture.index < 1.05 && 
currentGesture.middle >  
1.05 && currentGesture.ring > 1.05  
&& currentGesture.pinky > 1.05  
&& currentGesture.thumb < 1.05  
&&  currentGesture.angle >= 75 && currentGesture.angle <= 100);   
...
// Gesture D: Index and middle straight others 
bent   bool gesture5 = (currentGesture.index < 1.05 
&& currentGesture.middle < 1.05  
...
currentGesture.thumb > 1.05 &&   
currentGesture.angle >= 75 && currentGesture.angle <= 100);   
// all fingers bent except pinky   bool 
gesture7 = (currentGesture.index > 1.05 &&                   
currentGesture.middle > 1.05 &&                   
currentGesture.ring > 1.05 &&                    
currentGesture.pinky < 1.03 &&                    
currentGesture.thumb > 1.05 &&   
currentGesture.angle >= 75 && currentGesture.angle <= 100);   
// all fingers bent except index   bool 
gesture8 = ( 
currentGesture.index < 1.05 &&                   
currentGesture.middle > 1.05 &&                   
currentGesture.ring > 1.05 &&                    
...
currentGesture.thumb > 1.05 &&   
currentGesture.angle >= 75 && currentGesture.angle <= 100);   
// all fingers open    bool gesture9 = 
(currentGesture.index < 1.05 &&                    
currentGesture.middle < 1.05 &&                    
currentGesture.ring < 1.05 &&                    
currentGesture.pinky < 1.05 &&                    
...
Update last detected gesture (priority order if multiple match)   if 
(gesture1) {   
